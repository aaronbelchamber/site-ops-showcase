...
class ThemeUpdater:
    """
    Handles WordPress theme updates with health checks, backups, and automatic rollback.
    """
    ...
    def check_updates(self) -> Dict[str, Any]:
        try:
            themes = self.wp_cli.check_theme_updates()
            return {"updates_available": len(themes) > 0, "themes": themes}
        except Exception as e:
            return {"updates_available": False, "themes": [], "error": str(e)}
    ...
    def update(
        ...
        target_name = theme if theme else "all-themes"
        update_id = f"theme_update_{time.strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:4]}"
        ...
        if quality_checks:
            set_task_progress(f"Capturing baseline site health snapshot before updating theme(s) '{target_name}'...")
            try:
        ...
        if create_backup:
            ...
            try:
                backup_manifest = self.backup_manager.create_backup(
                    description=f"Pre-theme-update backup ({target_name})",
                    suffix="theme"
                )
        ...
        # 3. Perform update
        set_task_progress(f"Updating theme(s) '{target_name}' via WP-CLI...")
        if theme:
            args = ["theme", "update", theme]
        else:
        ...
        if not update_res.success:
            set_task_progress("Theme update command failed!")
            rollback_ok = False
            rollback_err = None
            if backup_id:
        ...
        set_task_progress(f"Theme(s) '{target_name}' updated successfully!")
        return {
            "update_id": update_id,
            "site_name": site_name,
            "timestamp": timestamp_val,
