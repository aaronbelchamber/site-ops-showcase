...
class TestBackupComponents(unittest.TestCase):
    ...
    def test_database_dump(self):
        ...
        def fake_execute_stream(command, local_path):
            with open(local_path, "wb") as f:
                f.write(b"mocked_dump_data" * 64)  # must clear MIN_PLAUSIBLE_DUMP_BYTES
    ...
    def test_database_dump_rejects_empty_output(self):
        """An empty dump must fail loudly rather than be recorded as complete.

        `mysqldump | gzip` reports gzip's exit status, so a failed dump exited
        0 and left a 20-byte empty gzip. Every database backup taken before
        2026-08-27 was empty for this reason and every one was marked
        completed.
        """
        db_backup = DatabaseBackup(self.mock_executor, self.db_config, self.temp_dir)

        def fake_empty_stream(command, local_path):
            with open(local_path, "wb") as f:
                f.write(gzip.compress(b""))  # exactly what a failed dump left behind: 20 bytes
            return CommandResult(exit_code=0, stdout="", stderr="", success=True)

        self.mock_executor.execute_stream.side_effect = fake_empty_stream
    ...
    def test_database_dump_uses_pipefail(self):
        """The pipeline must not mask a mysqldump failure behind gzip's exit code."""
        db_backup = DatabaseBackup(self.mock_executor, self.db_config, self.temp_dir)

        def fake_execute_stream(command, local_path):
            with open(local_path, "wb") as f:
                f.write(b"mocked_dump_data" * 64)
            return CommandResult(exit_code=0, stdout="", stderr="", success=True)

        self.mock_executor.execute_stream.side_effect = fake_execute_stream
        db_backup.dump()

        cmd = self.mock_executor.execute_stream.call_args[0][0]
        self.assertIn("pipefail", cmd)
    ...
    def test_asset_archive_windows_local_quotes_path_for_cmd_exe(self):
        """
        A locally-managed Windows site must quote wp_path for cmd.exe (double
        quotes), not shlex.quote's single-quote style -- cmd.exe doesn't treat
        single quotes as quoting, so a path with a space or backslash would
        otherwise break the tar command entirely.
        """
        from src.execution.local import LocalExecutor
        local_executor = LocalExecutor()
        ...
        asset_backup = AssetBackup(local_executor, "C:\\projects\\wordpress", self.temp_dir, include_media=True)
        asset_backup.archive()

        args, kwargs = local_executor.execute_stream.call_args
        cmd = args[0]
        self.assertIn('-C "C:\\projects\\wordpress"', cmd)
